...
while(cnt<48):
    b.append(cnt)
    cnt=cnt+1
# joint_names  is the array storing the names of the robot PR2 joints
...
class Nodo(object):
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            ...
            a1=self.body_pos #  reading from the /body/joint_states topic
            a2= pd.DataFrame(a1)
            a3=(a2.values)
            ...
            b1=self.giskard_pos #  reading from the /giskard/joint_states topic
            b2= pd.DataFrame(b1)
            b3=(b2.values)
            cnt=0
            for i in b3:
            ...
            while(cnt<45):
                c[cnt]=a[cnt]-b[cnt]
                if abs(c[cnt])>1:   # Threshold for difference = 1
